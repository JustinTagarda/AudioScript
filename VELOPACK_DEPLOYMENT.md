...
- Private source repo: your current `VoxTranscriber` repository.
...
1. Create a new public GitHub repository named `VoxTranscriber-Releases`.
...
4. In your private `VoxTranscriber` repository, open `Settings` -> `Secrets and variables` -> `Actions`.
...
6. Set the variable value to the full public release repository URL, for example:
   ...
   `https://github.com/YOUR_GITHUB_NAME/VoxTranscriber-Releases`
...
- repository access to `VoxTranscriber-Releases`
...
1. Update the `<Version>` value in [VoxTranscriber.csproj](VoxTranscriber.csproj).
...
7. Open the public `VoxTranscriber-Releases` repository.
...
1. Change the single `<Version>` value in [VoxTranscriber.csproj](VoxTranscriber.csproj).
2. Commit and push.
...
```powershell
./scripts/Publish-VelopackRelease.ps1 -ReleaseRepoUrl "https://github.com/YOUR_GITHUB_NAME/VoxTranscriber-Releases"
```
...
./scripts/Publish-VelopackRelease.ps1 `
  -ReleaseRepoUrl "https://github.com/YOUR_GITHUB_NAME/VoxTranscriber-Releases" `
  -Token "YOUR_TOKEN" `
  -PublishToGitHub
```
...
- The Velopack application id is `JustinTagarda.VoxTranscriber`. Do not change it after you ship your first real release.
